...
static __IO uint16_t AdcConvBuf[BAT_MAX_SAMPLEBUF][BAT_MAX_CHANNEL] = {0};
...
void HAL_ADC_MspInit( ADC_HandleTypeDef *hadc )
{
  ...
  if (hadc->Instance == BAT_ADCx) {
    ...
    /* DMA IT ********************************************************************/
    HAL_NVIC_SetPriority(BAT_ADCx_DMA_IRQn, BAT_ADCx_DMA_IRQn_PREEMPT, BAT_ADCx_DMA_IRQn_SUB);
...
void ADC_Config( void )
{
  ...
  AdcHandle.Init.NbrOfConversion       = BAT_MAX_CHANNEL;
  ...
  /* ADC Start *****************************************************************/
  if (HAL_ADC_Start_DMA(&AdcHandle, (uint32_t*)AdcConvBuf, BAT_MAX_CHANNEL * BAT_MAX_SAMPLEBUF) != HAL_OK)
    while (1) { ; }
}
...
{
  uint32_t tmp;
  for (uint8_t i = 0; i < BAT_MAX_CHANNEL; i++) {
    tmp = 0;
    for (uint8_t j = 0; j < BAT_MAX_SAMPLEBUF; j++) {
      tmp += AdcConvBuf[j][i];
    }
    aveAdc[i] = tmp >> 3;
  }
